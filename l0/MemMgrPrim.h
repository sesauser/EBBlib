...
CObjInterface(EBBMemMgr)
  EBBMemMgrPrim_ftable;

     static inline void
       EBBMemMgrPrimSetFT(EBBMemMgrPrimRef o) {
  o->ft = &EBBMemMgrPrim_ftable;
     }
     ...
     static inline EBBRC
       ...
       return EC(theEBBMemMgrPrimId)->alloc(EB(theEBBMemMgrPrimId), size, mem,
					    pool);
     }

     static inline EBBRC
       EBBPrimFree(void *mem) {
       return EC(theEBBMemMgrPrimId)->free(EB(theEBBMemMgrPrimId), mem);
     }
