...
lrt_pic_disable(uval vec) {
  if (vec != RST_VEC)
    lrt_pic_set_remove(pic.vecs[vec].set, lrt_pic_myid);
}
...
lrt_pic_init(uval numlpics, lrt_pic_handler h) {
  ...
  for (id = lrt_pic_firstid + 1; id <= lrt_pic_lastid; id++) {
    if (pthread_create
	((pthread_t *) (&lpics[id].aux), NULL,
	 (void *(*)(void *)) lrt_pic_loop, (void *) id) != 0) {
...
// the handler if the interrupt includes the pic id of the loop in its pic
// set
sval
lrt_pic_loop(lrt_pic_id myid) {
  ...
  // start with ipi and reset enabled on all lpics so that we can get going
  // via an ipi
  lrt_pic_enableipi();
