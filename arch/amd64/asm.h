...
/*
 * in order to force relocation for data to be 64 bit we need to use movabsq
 * instead of movq which indicate 32 bit range. we use gcc option
 * mcmodel=medium to get text reloc 32 bit and data 64 bit needed for us. 
 */
...
/*
 * we assume running gcc -fomit-frame-pointer with NDEBUG, otherwise we
 * compile with frame pointer to facilitate debugging (and profiling).
